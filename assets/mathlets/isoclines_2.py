...
def f_factory(a: float, b: float):
    # y' = y^2 + a sin(y) + b e^y
    return lambda x, y: y**2 + a*np.sin(y) + np.exp(b*y)

def slope_field(a: float, b: float, x_range=XRANGE, y_range=YRANGE, n=20, seg_len=0.30):
    f = f_factory(a, b)
...
def isocline_grid(a: float, b: float, x_range=XRANGE, y_range=YRANGE, n=120):
    f = f_factory(a, b)
...
def rk4_path(a: float, b: float, x0: float, y0: float, x_end: float, h=0.02, y_clip=20.0):
    f = f_factory(a, b)
